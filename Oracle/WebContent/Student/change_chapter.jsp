...
<body>
	<%
		...
		if (questionTypeChapter != null) {

			String ChapterID = questionTypeChapter.getChapterID();//章节号
			String ChapterName = questionTypeChapter.getChapterName();//章节名
			String CourseID =new String(questionTypeChapter.getCourseID());//课程编号
			String meaning = questionTypeChapter.getMeaning();
			String ProblemNumber="0";
			try{
				  ProblemNumber = new String(questionTypeChapter.getProblemNumber()); 
					...
					<tbody>
						<tr>
							<td><input type="text" class="form-control"
								style="width: 100%" name="ChapterID" value=<%=ChapterID%> readonly="readonly"></td>
							<td><input type="text" class="form-control"
								style="width: 100%" name="ChapterName" value=<%=ChapterName%>></td>
							<td><input type="text" class="form-control"
								style="width: 100%" name="ProblemNumber" value=<%=ProblemNumber%>></td>
							<td><select name="courseselect" >
									<option value="1104201509"<%if("'1104201509'".equals(CourseID)){%> selected<%} %>>操作系统</option>
				...
				<div class="row">
					...
					<div class="col-md-2">
						<button type="button" class="btn btn-primary" id="cancel"
							onclick="location.href = '/Oracle/ChapterinfoServlet'">取消</button>
					</div>
					<div class="col-md-4"></div>
				</div>
				<%
